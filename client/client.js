window.__ModuleLoader__.load({ id: "dshmarket", factory: (require) => {
		...
		const zh = {
			...
			aiFixIfSelf: "【是 → 你就是这个 harness】严禁原地改动组合、harness 或核心包：不写 cordis.patch.yml、不动 bundle 顺序/启停、不装/卸/升级插件、不重装依赖、不升级/重启 dsh。三步：① 先只读分析，不改任何东西；② 给出 apply.sh/.bat（本次修复的改动）与 rollback.sh/.bat（完整撤销 apply）；两者都幂等，apply 开头先快照备份，rollback 从快照恢复——apply 跑到一半失败也能回滚；③ 让我在外部终端跑 apply，并把 stdout/stderr 拷回给你判断。每步先说明理由、等我确认；绝不自行执行。",
