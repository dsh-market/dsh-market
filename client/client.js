window.__ModuleLoader__.load({ id: "dshmarket", factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			hostCompatibilityTitle: "⚠️ 兼容性检查",
			hostIncompatibleInstallWarning: "当前 DSH {host} 暂不支持最新版 {latest}。",
			hostIncompatibleInstallFound: "可安装的最新兼容版本：{version}",
			hostIncompatibleUnknownVersion: "未知版本",
			hostIncompatibleUpdateFound: "此 DSH 支持的最新兼容版本为 {version}。",
			hostIncompatibleNoCompatUpdate: "无法更新，该插件最新版本与当前 DSH 版本不兼容。",
			hostIncompatibleCurrentVersionUnknown: "当前版本",
			hostIncompatibleSearchError: "暂时无法查询 {plugin} 的兼容版本，请稍后重试。",
			...
			hostIncompatibleClose: "关闭",
			hostIncompatibleCancel: "取消安装",
			hostIncompatibleSearching: "正在查找当前 DSH 可安装的最新兼容版本…",
			hostIncompatibleNoCompat: "目前没有找到适用于当前 DSH 的兼容版本。",
		...
		const en = {
			...
			hostCompatibilityTitle: "⚠️ Compatibility check",
			hostIncompatibleInstallWarning: "DSH {host} does not support the latest version ({latest}).",
			hostIncompatibleInstallFound: "Latest compatible version available: {version}",
			hostIncompatibleUnknownVersion: "unknown version",
			hostIncompatibleUpdateFound: "The latest version compatible with this DSH is {version}.",
			hostIncompatibleNoCompatUpdate: "Cannot update: the latest version of this plugin is incompatible with the current DSH version.",
			hostIncompatibleCurrentVersionUnknown: "the installed version",
			hostIncompatibleSearchError: "Could not check compatible versions of {plugin}. Please try again.",
			...
			hostIncompatibleClose: "Close",
			hostIncompatibleCancel: "Cancel",
			hostIncompatibleSearching: "Finding the latest version compatible with this DSH…",
			hostIncompatibleNoCompat: "No version compatible with this DSH was found.",
		...
		*/
		...
		var Market_module_css_default = {
			...
			"hostCompatPrimary": "nUhMVa_hostCompatPrimary",
			"hostCompatText": "nUhMVa_hostCompatText",
		...
		*/
		...
		function MarketSection(props) {
			...
			* this set. */
			...
			/** A release whose declared host requirement this host does not meet (#404). */
			const [hostIncompatible, setHostIncompatible] = (0, react.useState)(null);
			const [findingCompat, setFindingCompat] = (0, react.useState)({ status: "loading" });
			...
			(0, react.useEffect)(() => {
				...
				fetch(api("/dsh-market/find-compatible"), {
					...
					if (typeof data.compatibleVersion === "string" && data.compatibleVersion !== "") setFindingCompat({
						status: "found",
						version: data.compatibleVersion
					});
					else setFindingCompat({ status: data.reason === "installed-version-unknown" ? "error" : "not-found" });
				}).catch(() => {
			...
			* backup export) and falls back to the caller's name. */
			...
			const doInstall = (0, react.useCallback)((plugin, version) => {
					...
					if (status === 200 && body.ok) {
						...
						if (body.hostIncompatible && typeof body.hostIncompatible === "object") {
							...
							setHostIncompatible({
								...
								latestVersion: null,
								hostVersion: null,
								currentVersion: typeof notice.currentVersion === "string" ? notice.currentVersion : updates[name]?.current ?? updates[name]?.version ?? null,
								plugin: null
							});
							return;
						}
					...
					hostIncompatible !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
						...
						footer: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
								...
								children: t(hostIncompatible.kind === "install" ? "hostIncompatibleCancel" : findingCompat.status === "not-found" ? "hostIncompatibleClose" : "hostIncompatibleKeep")
							}),
							...
							findingCompat.status === "error" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
								variant: "primary",
								onClick: () => setCompatRetry((value) => value + 1),
								children: t("hostIncompatibleRetry")
							})
						] }),
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: Market_module_css_default.hostCompatText,
							children: [hostIncompatible.kind === "install" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: t("hostIncompatibleInstallWarning").replace("{host}", hostIncompatible.hostVersion ?? t("hostIncompatibleUnknownVersion")).replace("{latest}", hostIncompatible.latestVersion ?? t("hostIncompatibleUnknownVersion")) }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: findingCompat.status === "found" ? Market_module_css_default.hostCompatPrimary : void 0,
								role: "status",
								children: t(hostIncompatible.kind === "install" ? findingCompat.status === "found" ? "hostIncompatibleInstallFound" : findingCompat.status === "loading" ? "hostIncompatibleSearching" : findingCompat.status === "error" ? "hostIncompatibleSearchError" : "hostIncompatibleNoCompat" : findingCompat.status === "found" ? "hostIncompatibleUpdateFound" : findingCompat.status === "loading" ? "hostIncompatibleSearching" : findingCompat.status === "error" ? "hostIncompatibleSearchError" : "hostIncompatibleNoCompatUpdate").replace("{plugin}", hostIncompatible.name).replace("{version}", findingCompat.status === "found" ? findingCompat.version : hostIncompatible.currentVersion ?? t("hostIncompatibleCurrentVersionUnknown"))
							})]
						})
					}),
