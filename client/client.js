window.__ModuleLoader__.load({ id: "dshmarket", factory: (require) => {
		...
		const zh = {
			...
			aiFixIfSelf: "【是 → 只写脚本，请到外部执行；严禁在 harness 内改动】你正在修复的正是自己运行其中的 harness：严禁任何改动运行中组合、或触碰 harness / 核心包本身的操作——包括但不限于改写 cordis.patch.yml、增删或重排 dsh.profile.bundles、停用/启用 bundle、安装/更新/移除插件、重装（pnpm install / reinstall）依赖、升级或重启 DeepSeek Harness 及其任何 @deepseek-ai/* 核心包。请在当前会话工作区生成两份可直接执行的脚本文件（Windows 用 .bat，其它平台用 .sh），不要只贴命令：1) apply.sh 或 apply.bat：涵盖本次诊断需做的全部改动（尽量幂等、可重复执行）；2) rollback.sh 或 rollback.bat：能完整撤销 apply 的一切改动，恢复到动手前状态。两份脚本都要在开头用注释写清楚：它在改什么、是否有风险、是否需要先停掉当前 DSH；关键改动用多个幂等命令原子化，避免不可逆操作。脚本内容也一并展示给我，并告诉我这两个文件的位置。然后由我在外部终端运行 apply.sh（或 apply.bat），并把它的 stdout/stderr 原样拷回给你；你据此判断是否成功、是否要微调脚本、或触发 rollback.sh 回滚。每一步都先说明理由并等我确认后再继续，绝不要在当前会话里自行执行这些改动。",
		...
		const en = {
			...
			aiFixIfSelf: "If YES — you are fixing the harness you are running inside: do NOT touch the live composition or the harness / core packages in any way. This includes, but is not limited to, editing cordis.patch.yml, adding / removing / reordering dsh.profile.bundles, enabling / disabling a bundle, installing / updating / removing a plugin, reinstalling dependencies (pnpm install / reinstall), or upgrading / restarting DeepSeek Harness or any @deepseek-ai/* core package. In the current session's workspace, create two ready-to-run script files (.bat on Windows, .sh elsewhere) — do not only paste commands: 1) apply.sh / apply.bat — the full set of changes this diagnosis needs, as idempotent and re-runnable as possible; 2) rollback.sh / rollback.bat — completely reverts everything apply did, back to the pre-change state. Both scripts start with a header comment stating what they change, the risk, and whether the current DSH must be stopped first; make key steps atomic with idempotent commands and avoid irreversible operations. Show me the script contents too and tell me where the files live. Then I run apply.sh (or apply.bat) in an EXTERNAL terminal and paste its stdout/stderr back to you; you judge whether it succeeded, whether the script needs tuning, or whether to trigger rollback.sh. Explain the reason and wait for my confirmation before each step — never run these changes yourself in this session.",
		...
		*/
		...
		const css = ".BOT17a_root{min-width:0;height:100%;color:var(--dsw-alias-label-primary,#1f2328);flex-direction:column;display:flex;position:relative}.BOT17a_head{flex-direction:column;gap:12px;padding:4px 4px 6px;display:flex}.BOT17a_title{margin:0;font-size:16px;font-weight:500;line-height:24px}.BOT17a_sub{color:var(--dsw-alias-label-tertiary,#8b93a1);align-items:center;gap:8px;margin:0;font-size:12px;line-height:18px;display:flex}.BOT17a_submitLink{color:var(--dsw-alias-label-tertiary,#8b93a1);white-space:nowrap;font-size:11px;line-height:18px;text-decoration:none}.BOT17a_submitLink:hover{color:var(--dsw-alias-brand-primary,#4f6ef7);text-decoration:underline}.BOT17a_tabs{border-bottom:1px solid var(--dsw-alias-border-l2,#e5e7eb);align-items:flex-end;gap:2px;display:flex}.BOT17a_tab{font:inherit;color:var(--dsw-alias-label-secondary,#6b7280);cursor:pointer;white-space:nowrap;background:0 0;border:none;border-bottom:2px solid #0000;padding:7px 12px;font-size:13px}.BOT17a_tab.BOT17a_on{color:var(--dsw-alias-brand-primary,#4f6ef7);border-bottom-color:var(--dsw-alias-brand-primary,#4f6ef7);font-weight:600}.BOT17a_subTabs{align-items:flex-end;gap:2px;margin:-4px 0 4px;display:flex}.BOT17a_banner{background:var(--dsw-alias-bg-layer-2,#fdf3e3);border:1px solid var(--dsw-alias-border-l2,#f3e3c3);border-radius:8px;align-items:center;gap:8px;margin:0;padding:8px 12px;font-size:12px;display:flex}.BOT17a_bannerIcon{color:var(--dsw-alias-label-secondary,#6b7280);flex-shrink:0}.BOT17a_bannerHint{color:var(--dsw-alias-label-tertiary,#8b93a1);cursor:help;display:inline-flex}.BOT17a_body{flex:1;padding:12px 4px 24px;overflow-x:hidden;overflow-y:auto}.BOT17a_stickyHead{z-index:5;background:var(--dsw-alias-bg-layer-2,#f7f8fa);position:sticky;top:-1px}.BOT17a_stickyHead:before{content:\"\";background:inherit;pointer-events:none;height:14px;position:absolute;bottom:100%;left:0;right:0}.BOT17a_cats{margin:0 -4px 2px;padding:12px 4px 4px}.BOT17a_catsRow{align-items:flex-start;gap:8px;display:flex;position:relative}.BOT17a_star{color:var(--dsw-alias-label-secondary,#9ca3af);white-space:nowrap;flex:none;font-size:11px}.BOT17a_top{z-index:20;display:inline-flex;position:absolute;bottom:18px;right:18px}.BOT17a_topBtn{border-radius:99px;width:38px;height:38px;padding:0}.BOT17a_tag{border:1px solid var(--dsw-alias-border-l3,#d9dde3);color:var(--dsw-alias-label-secondary,#6b7280);border-radius:4px;flex-shrink:0;padding:1px 6px;font-size:11px;line-height:16px}.BOT17a_okState{color:var(--dsw-alias-state-success-primary,#16a34a);white-space:nowrap;font-size:12px;font-weight:600}.BOT17a_catsWrap{flex-wrap:wrap;flex:1;align-items:center;gap:6px;min-width:0;display:flex}.BOT17a_catsCollapsed{max-height:62px;overflow:hidden}.BOT17a_catsToggle.BOT17a_catsToggle{height:26px;min-height:26px;color:var(--dsw-alias-label-secondary,#6b7280);padding:0 6px}.BOT17a_shots{-webkit-overflow-scrolling:touch;scrollbar-width:thin;gap:8px;margin:6px 0 8px;padding:2px 0 6px;display:flex;overflow-x:auto}.BOT17a_shot{object-fit:cover;border:1px solid var(--dsw-alias-border-default,#e5e7eb);background:var(--dsw-alias-bg-layer-2,#f3f4f6);cursor:pointer;border-radius:8px;flex:none;width:220px;height:150px}.BOT17a_cardShots{-webkit-overflow-scrolling:touch;scrollbar-width:thin;gap:6px;margin:0 0 6px;padding:0 0 2px;display:flex;overflow-x:auto}.BOT17a_cardShot{object-fit:contain;border:1px solid var(--dsw-alias-border-default,#e5e7eb);background:var(--dsw-alias-bg-layer-2,#f3f4f6);cursor:pointer;border-radius:8px;flex:none;width:132px;height:88px;display:block}.BOT17a_lightbox{z-index:10000;cursor:zoom-out;background:#000000d9;justify-content:center;align-items:center;display:flex;position:fixed;top:0;bottom:0;left:0;right:0}.BOT17a_lightboxImg{object-fit:contain;cursor:default;border-radius:4px;max-width:90vw;max-height:85vh}.BOT17a_lightboxClose{color:#fff;cursor:pointer;background:#ffffff1f;border:none;border-radius:99px;place-items:center;width:36px;height:36px;font-size:22px;line-height:1;display:grid;position:absolute;top:16px;right:16px}.BOT17a_lightboxClose:hover{background:#ffffff38}.BOT17a_lightboxNav{color:#fff;cursor:pointer;background:#ffffff1f;border:none;border-radius:99px;place-items:center;width:44px;height:44px;display:grid;position:absolute;top:50%;transform:translateY(-50%)}.BOT17a_lightboxNav:hover{background:#ffffff38}.BOT17a_lightboxPrev{left:16px}.BOT17a_lightboxNext{right:16px}.BOT17a_lightboxDots{gap:8px;display:flex;position:absolute;bottom:20px;left:50%;transform:translate(-50%)}.BOT17a_lightboxDot{cursor:pointer;background:#fff6;border-radius:99px;width:7px;height:7px}.BOT17a_lightboxDotOn{background:#fff}.BOT17a_cmd{background:var(--dsw-alias-bg-layer-2,#f3f4f6);word-break:break-all;border-radius:6px;margin:8px 0 0;padding:8px 10px;font-family:ui-monospace,Menlo,monospace;font-size:11px;line-height:18px}.BOT17a_warnLine{color:var(--dsw-alias-state-warn-primary,#b45309);flex-wrap:wrap;align-items:center;gap:4px;margin:0;font-size:12px;font-weight:600;line-height:18px;display:flex}.BOT17a_modalNote{color:var(--dsw-alias-label-tertiary,#8b93a1);align-items:center;gap:4px;margin:12px 0 0;font-size:12px;line-height:18px;display:flex}.BOT17a_grid{grid-template-columns:repeat(2,minmax(0,1fr));align-items:start;gap:10px;display:grid}.BOT17a_masonry{align-items:flex-start;gap:10px;display:flex}.BOT17a_masonryCol{flex-direction:column;flex:1;gap:10px;min-width:0;display:flex}.BOT17a_masonry>.BOT17a_masonryCol>*{align-self:stretch;min-width:0}@media (max-width:680px){.BOT17a_masonry{flex-direction:column}}.BOT17a_pairGrid{grid-template-columns:repeat(2,minmax(0,1fr));align-items:start;gap:10px;display:grid}@media (max-width:680px){.BOT17a_grid,.BOT17a_pairGrid{grid-template-columns:minmax(0,1fr)}}.BOT17a_swatches{border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:8px;gap:0;height:34px;display:flex;overflow:hidden}.BOT17a_themesGrid{margin-bottom:12px}.BOT17a_swatches i{flex:1}.BOT17a_card{background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:12px;flex-direction:column;align-self:start;gap:12px;padding:12px 14px;display:flex}.BOT17a_row1{align-items:flex-start;gap:10px;min-width:0;display:flex}.BOT17a_cardAction{flex-shrink:0;align-items:center;display:inline-flex}.BOT17a_installBtn.BOT17a_installBtn{min-width:64px}.BOT17a_av{color:#fff;object-fit:cover;background:var(--dsw-alias-bg-layer-2,#f3f4f6);border-radius:50%;flex-shrink:0;place-items:center;width:16px;height:16px;font-size:9px;font-weight:700;display:grid}.BOT17a_nm{text-overflow:ellipsis;white-space:nowrap;font-size:15px;font-weight:600;line-height:22px;overflow:hidden}.BOT17a_nmLink{color:inherit;text-decoration:none;display:block}.BOT17a_nmLink:hover{color:var(--dsw-alias-brand-primary,#4f6ef7);text-decoration:underline}.BOT17a_byline{flex-wrap:wrap;align-items:center;gap:6px;min-width:0;margin-top:2px;display:flex}.BOT17a_meta{color:var(--dsw-alias-label-secondary,#9ca3af);margin-top:2px;font-size:11px}.BOT17a_metaInline{color:var(--dsw-alias-label-secondary,#9ca3af);font-size:11px}.BOT17a_owner{color:var(--dsw-alias-label-secondary,#9ca3af);text-overflow:ellipsis;white-space:nowrap;flex:0 auto;min-width:44px;font-size:11px;overflow:hidden}.BOT17a_desc{color:var(--dsw-alias-label-tertiary,#8b93a1);margin:0;font-size:12px;line-height:18px}.BOT17a_descClamp{-webkit-line-clamp:5;-webkit-box-orient:vertical;display:-webkit-box;overflow:hidden}.BOT17a_descToggle{width:20px;height:16px;color:var(--dsw-alias-label-tertiary,#8b93a1);cursor:pointer;background:0 0;border:none;justify-content:center;align-items:center;margin-top:2px;padding:0;display:flex}.BOT17a_descToggle:hover{color:var(--dsw-alias-brand-primary,#4f6ef7)}.BOT17a_foot{flex-wrap:wrap;align-items:center;gap:8px;display:flex}.BOT17a_foot .BOT17a_metaInline,.BOT17a_foot .BOT17a_src{white-space:nowrap}.BOT17a_grow{flex:1}.BOT17a_titleRow{align-items:center;gap:10px;display:flex}.BOT17a_version{color:var(--dsw-alias-label-tertiary,#8b93a1);font-variant-numeric:tabular-nums;flex-shrink:0;font-size:12px;line-height:20px}.BOT17a_repoLink{color:var(--dsw-alias-label-tertiary,#8b93a1);flex-shrink:0;font-size:12px;line-height:20px;text-decoration:none}.BOT17a_repoLink:hover{color:var(--dsw-alias-brand-primary,#4f6ef7)}.BOT17a_descTight{min-height:0}.BOT17a_src{color:var(--dsw-alias-label-secondary,#9ca3af);font-size:11px;text-decoration:none}.BOT17a_src:hover{color:var(--dsw-alias-brand-primary,#4f6ef7)}.BOT17a_dot{vertical-align:2px;margin-left:5px}.BOT17a_act{flex-wrap:wrap;align-items:center;gap:6px;margin-top:6px;font-size:11px;display:flex}.BOT17a_actLive{color:var(--dsw-alias-state-success-primary,#16a34a);align-items:center;gap:4px;font-weight:600;display:inline-flex}.BOT17a_actWarn{color:var(--dsw-alias-state-warn-primary,#b45309);align-items:center;gap:4px;font-weight:600;display:inline-flex}.BOT17a_actBroken{color:var(--dsw-alias-state-error-primary,#dc2626);align-items:center;gap:4px;font-weight:600;display:inline-flex}.BOT17a_actWhy{color:var(--dsw-alias-label-secondary,#6b7280);margin-top:2px}.BOT17a_loading{color:var(--dsw-alias-label-secondary,#9ca3af);flex-direction:column;align-items:center;gap:12px;padding:48px;font-size:13px;display:flex}.BOT17a_spin{color:var(--dsw-alias-brand-primary,#4f6ef7);flex-shrink:0;animation:.8s linear infinite BOT17a_sp;display:inline-flex}.BOT17a_logoMark{color:var(--dsw-alias-brand-primary,#4f6ef7);flex-shrink:0;display:inline-flex}.BOT17a_logoPlug{transform-box:fill-box;transform-origin:50%;animation:1.5s cubic-bezier(.4,0,.2,1) infinite BOT17a_dshmPlug}@keyframes BOT17a_dshmPlug{0%,12%{transform:rotate(9deg)}45%,62%{transform:translate(-1.28px,1.27px)rotate(0)}95%,to{transform:rotate(9deg)}}@media (prefers-reduced-motion:reduce){.BOT17a_logoPlug,.BOT17a_spin{animation:1.5s ease-in-out infinite BOT17a_dshmPlugFade}}@keyframes BOT17a_dshmPlugFade{0%,to{opacity:1}50%{opacity:.35}}@keyframes BOT17a_sp{to{transform:rotate(360deg)}}.BOT17a_progress{background:var(--dsw-alias-bg-layer-2,#f3f4f6);border:1px solid var(--dsw-alias-border-l2,#e5e7eb);color:var(--dsw-alias-label-secondary,#6b7280);border-radius:8px;flex-wrap:wrap;align-items:center;gap:9px;margin:0;padding:8px 12px;font-size:12px;display:flex}.BOT17a_bar{background:var(--dsw-alias-border-l1,#e5e7eb);border-radius:99px;width:100%;height:4px;overflow:hidden}.BOT17a_barFill{background:var(--dsw-alias-brand-primary,#4f6ef7);border-radius:99px;height:100%;transition:width .6s}.BOT17a_barWave{width:30%;animation:1.2s ease-in-out infinite BOT17a_dshmSlide}@keyframes BOT17a_dshmSlide{0%{margin-left:-30%}to{margin-left:100%}}.BOT17a_irow .BOT17a_progress{margin-top:8px}.BOT17a_progress code{text-overflow:ellipsis;white-space:nowrap;font-family:ui-monospace,Menlo,monospace;font-size:11px;overflow:hidden}.BOT17a_empty{color:var(--dsw-alias-label-secondary,#9ca3af);text-align:center;padding:32px;font-size:13px}.BOT17a_err{color:var(--dsw-alias-state-error-primary,#dc2626);white-space:pre-wrap;word-break:break-all;margin:8px 0;font-size:12px}.BOT17a_cardBlocked{border-color:var(--dsw-alias-state-error-primary,#dc2626)}.BOT17a_conflictHead{align-items:flex-start;gap:8px;display:flex}.BOT17a_conflictIcon{color:var(--dsw-alias-state-error-primary,#dc2626);flex-shrink:0;margin-top:1px}.BOT17a_conflictTitle{color:var(--dsw-alias-state-error-primary,#dc2626);font-size:13px;font-weight:600;line-height:18px}.BOT17a_conflictBody{margin:0;font-size:12px;line-height:19px}.BOT17a_roster{background:var(--dsw-alias-border-l2,#e5e7eb);border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:8px;flex-direction:column;gap:1px;display:flex;overflow:hidden}.BOT17a_rosterRow{background:var(--dsw-alias-bg-layer-1,#fff);align-items:center;gap:8px;padding:7px 10px;display:flex}.BOT17a_rosterMain{flex-direction:column;min-width:0;display:flex}.BOT17a_rosterName{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-size:12px;font-weight:600;overflow:hidden}.BOT17a_rosterAuthor{color:var(--dsw-alias-label-secondary,#9ca3af);text-overflow:ellipsis;white-space:nowrap;font-size:10.5px;overflow:hidden}.BOT17a_rosterTag{border-radius:4px;flex-shrink:0;margin-left:auto;padding:1px 6px;font-size:10.5px;font-weight:600}.BOT17a_rosterTagKeep{color:var(--dsw-alias-state-success-primary,#16a34a);background:#16a34a1f}.BOT17a_rosterTagDrop{color:var(--dsw-alias-state-error-primary,#dc2626);background:#dc26261f}.BOT17a_rosterRowOut .BOT17a_rosterName{text-decoration:line-through}.BOT17a_rosterRowOut{opacity:.62}.BOT17a_rosterSplit{background:var(--dsw-alias-border-l2,#e5e7eb);height:1px}.BOT17a_reassure{color:var(--dsw-alias-label-secondary,#6b7280);align-items:center;gap:5px;margin:0;font-size:11.5px;line-height:17px;display:flex}.BOT17a_reassureOk{color:var(--dsw-alias-state-success-primary,#16a34a);flex-shrink:0}.BOT17a_conflictWhy{color:var(--dsw-alias-label-tertiary,#8b93a1);overflow-wrap:anywhere;margin-top:2px;font-family:ui-monospace,Menlo,monospace;font-size:11px;line-height:17px}.BOT17a_conflictWhyText{margin-top:5px;font-family:-apple-system,BlinkMacSystemFont,PingFang SC,sans-serif}.BOT17a_choices{flex-direction:column;gap:7px;display:flex}.BOT17a_choice{text-align:left;font:inherit;cursor:pointer;background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l3,#d9dde3);border-radius:9px;align-items:flex-start;gap:9px;padding:9px 11px;display:flex}.BOT17a_choice:has(input:disabled){cursor:default;opacity:.6}.BOT17a_choiceOn{border-color:var(--dsw-alias-brand-primary,#4f6ef7);background:var(--dsw-alias-bg-layer-2,#f5f7ff)}.BOT17a_choiceRadio{width:13px;height:13px;accent-color:var(--dsw-alias-brand-primary,#4f6ef7);flex-shrink:0;margin:2px 0 0}.BOT17a_choiceMain{flex-direction:column;gap:3px;min-width:0;display:flex}.BOT17a_choiceTitle{font-size:12px;font-weight:600;line-height:17px}.BOT17a_choiceNote{color:var(--dsw-alias-label-tertiary,#8b93a1);font-size:11px;line-height:16px}.BOT17a_choiceSafe{color:var(--dsw-alias-state-success-primary,#16a34a)}.BOT17a_stateTag{white-space:nowrap;background:var(--dsw-alias-bg-layer-2,#f3f4f6);min-height:20px;color:var(--dsw-alias-label-secondary,#6b7280);border-radius:5px;flex-shrink:0;align-items:center;gap:5px;padding:1px 7px;font-size:11px;line-height:16px;display:inline-flex}.BOT17a_stateTag[data-on=true]{color:var(--dsw-alias-state-success-primary,#16a34a);background:color-mix(in srgb, var(--dsw-alias-state-success-primary,#16a34a) 10%, transparent)}.BOT17a_stateDot{background:var(--dsw-alias-label-tertiary,#8b93a1);border-radius:999px;flex:none;width:6px;height:6px}.BOT17a_stateDot[data-on=true]{background:var(--dsw-alias-state-success-primary,#16a34a)}.BOT17a_metaTag{text-overflow:ellipsis;white-space:nowrap;background:var(--dsw-alias-bg-layer-2,#f3f4f6);min-width:0;max-width:100%;min-height:20px;color:var(--dsw-alias-label-tertiary,#8b93a1);border-radius:5px;flex-shrink:1;padding:1px 7px;font-size:11px;line-height:18px;display:inline-block;overflow:hidden}.BOT17a_metaTagOk{color:var(--dsw-alias-state-success-primary,#16a34a);background:color-mix(in srgb, var(--dsw-alias-state-success-primary,#16a34a) 10%, transparent)}.BOT17a_nameLink{color:inherit;text-decoration:none}.BOT17a_irowName{text-overflow:clip;align-items:baseline;gap:6px;min-width:0;display:flex;overflow:visible}.BOT17a_irowNameText{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}.BOT17a_irowName>.BOT17a_owner{flex:none}.BOT17a_nameLink:hover{color:var(--dsw-alias-brand-primary,#4f6ef7);text-decoration:underline}.BOT17a_opWrap{flex-shrink:0;display:inline-flex;position:relative}.BOT17a_opEntry{font:inherit;cursor:pointer;white-space:nowrap;color:var(--dsw-alias-label-primary,#1f2328);background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l3,#d9dde3);border-radius:7px;align-items:center;gap:6px;padding:4px 10px;font-size:12px;display:inline-flex;position:relative}.BOT17a_opEntryQuiet{color:var(--dsw-alias-label-secondary,#6b7280);background:0 0;border-color:#0000}.BOT17a_opEntryAlert{border-color:var(--dsw-alias-state-error-primary,#dc2626);color:var(--dsw-alias-state-error-primary,#dc2626)}.BOT17a_opDot{background:var(--dsw-alias-state-error-primary,#dc2626);border-radius:99px;width:8px;height:8px;position:absolute;top:-3px;right:-3px}.BOT17a_opPanel{z-index:40;background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l3,#d9dde3);border-radius:12px;width:460px;max-width:86vw;max-height:70vh;position:absolute;top:calc(100% + 6px);right:0;overflow-y:auto;box-shadow:0 20px 52px #00000047}.BOT17a_opHead{border-bottom:1px solid var(--dsw-alias-border-l2,#e5e7eb);align-items:center;gap:8px;padding:9px 14px;display:flex}.BOT17a_opPanelTitle{font-size:12.5px;font-weight:600}.BOT17a_opCloseBtn.BOT17a_opCloseBtn{min-width:0;color:var(--dsw-alias-label-secondary,#6b7280);padding:0 6px}.BOT17a_opAggregate{border-bottom:1px solid var(--dsw-alias-border-l2,#e5e7eb);background:var(--dsw-alias-bg-layer-2,#f7f8fa);padding:9px 14px}.BOT17a_opAggregateTop{font-variant-numeric:tabular-nums;font-size:12px;font-weight:600}.BOT17a_opAggregateHint{color:var(--dsw-alias-label-tertiary,#8b93a1);margin-top:4px;font-size:10.5px}.BOT17a_opRow{border-bottom:1px solid var(--dsw-alias-border-l2,#e5e7eb);align-items:flex-start;gap:9px;padding:9px 14px;display:flex}.BOT17a_opRow:last-child{border-bottom:none}.BOT17a_opRowAlert{background:#dc26260f}.BOT17a_opIcon{flex-shrink:0;place-items:center;width:14px;margin-top:1px;display:grid}.BOT17a_opQueuedIcon{color:var(--dsw-alias-label-tertiary,#8b93a1);font-size:12px}.BOT17a_opMain{flex-direction:column;flex:1;gap:3px;min-width:0;display:flex}.BOT17a_opTop{align-items:baseline;gap:6px;min-width:0;display:flex}.BOT17a_opVerb{color:var(--dsw-alias-label-secondary,#6b7280);flex-shrink:0;font-size:12px}.BOT17a_opName{text-overflow:ellipsis;white-space:nowrap;font-size:12px;font-weight:600;overflow:hidden}.BOT17a_opStatus{color:var(--dsw-alias-label-tertiary,#8b93a1);overflow-wrap:anywhere;font-size:10.5px;line-height:16px}.BOT17a_opStatusBad{color:var(--dsw-alias-state-error-primary,#dc2626)}.BOT17a_opActions{flex-shrink:0;align-items:center;gap:6px;margin-top:1px;display:flex}.BOT17a_opDecision{border-top:1px dashed var(--dsw-alias-border-l2,#e5e7eb);flex-direction:column;gap:8px;margin-top:8px;padding-top:8px;display:flex}.BOT17a_opDecisionFoot{align-items:center;gap:8px;display:flex}.BOT17a_conflictDetailsToggle{font:inherit;cursor:pointer;color:var(--dsw-alias-label-tertiary,#8b93a1);background:0 0;border:none;align-items:center;gap:4px;padding:2px 0;font-size:11px;display:inline-flex}.BOT17a_conflictDetailsToggle:hover{color:var(--dsw-alias-label-secondary,#6b7280)}.BOT17a_opEmpty{text-align:center;color:var(--dsw-alias-label-secondary,#6b7280);padding:30px 14px;font-size:12px}.BOT17a_opEmptyHint{color:var(--dsw-alias-label-tertiary,#8b93a1);margin-top:4px;font-size:11px}.BOT17a_cardBlockedMark{font:inherit;cursor:pointer;color:var(--dsw-alias-state-error-primary,#dc2626);border:1px solid var(--dsw-alias-state-error-primary,#dc2626);background:#dc262614;border-radius:7px;align-items:center;gap:5px;padding:4px 9px;font-size:11px;display:inline-flex}.BOT17a_dangerBtn.BOT17a_dangerBtn{color:var(--dsw-alias-state-error-primary,#dc2626);border-color:var(--dsw-alias-state-error-primary,#dc2626)}.BOT17a_dangerBtn.BOT17a_dangerBtn:hover:not(:disabled){background:var(--dsw-alias-state-error-primary,#dc2626);color:#fff}.BOT17a_dangerArmed.BOT17a_dangerArmed{background:var(--dsw-alias-state-error-primary,#dc2626);border-color:var(--dsw-alias-state-error-primary,#dc2626);color:#fff}.BOT17a_retryBtn{margin-top:4px}.BOT17a_irow{background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:10px;flex-direction:column;gap:10px;min-width:0;padding:12px 14px;display:flex}.BOT17a_irowActions{flex-wrap:wrap;justify-content:flex-start;align-items:center;gap:8px;min-width:0;display:flex}.BOT17a_irowTrailing{flex-wrap:nowrap;align-items:center;gap:8px;min-width:0;display:inline-flex}.BOT17a_irowMissing{filter:grayscale();opacity:.5}.BOT17a_irow>.BOT17a_src,.BOT17a_irow>.BOT17a_owner,.BOT17a_irow button{white-space:nowrap;flex-shrink:0}.BOT17a_tabSearchRow{padding:0 4px 6px;display:flex}.BOT17a_tabSearch{width:100%}.BOT17a_spec{color:var(--dsw-alias-label-secondary,#9ca3af);overflow-wrap:anywhere;min-width:0;font-family:ui-monospace,Menlo,monospace;font-size:11px}.BOT17a_specTag{white-space:nowrap;border-radius:4px;flex-shrink:0;align-items:center;height:16px;padding:0 5px;font-size:10px;font-weight:600;display:inline-flex}.BOT17a_specTagGit{color:#0b7285;background:#12a3c41f}.BOT17a_specTagFile{color:#b07d1b;background:#f0b42924}.BOT17a_backupCheckList .BOT17a_grow{white-space:nowrap;text-overflow:ellipsis;flex:70%;min-width:0;overflow:hidden}.BOT17a_backupCheckList .BOT17a_spec{text-align:right;white-space:nowrap;text-overflow:ellipsis;flex:0 30%;max-width:30%;overflow:hidden}.BOT17a_staleAction{margin-top:8px}.BOT17a_pct{color:var(--dsw-alias-label-secondary,#6b7280);flex-shrink:0;font-size:11px;font-weight:600}.BOT17a_pager{flex-wrap:wrap;justify-content:space-between;align-items:center;gap:12px;margin:16px 0 4px;display:flex}.BOT17a_pagerPages{flex-wrap:wrap;flex:1;justify-content:center;align-items:center;gap:6px;min-width:0;display:flex}.BOT17a_pageEllipsis{color:var(--dsw-alias-label-secondary,#9ca3af);padding:0 2px;font-size:12px}.BOT17a_pageInfo{color:var(--dsw-alias-label-secondary,#6b7280);white-space:nowrap;font-size:12px}.BOT17a_depBadge{border:1px solid var(--dsw-alias-state-warn-primary,#b45309);color:var(--dsw-alias-state-warn-primary,#b45309);white-space:nowrap;border-radius:4px;flex-shrink:0;margin-left:6px;padding:1px 6px;font-size:11px;font-weight:600;line-height:16px}.BOT17a_deprecate{color:var(--dsw-alias-state-warn-primary,#b45309);background:var(--dsw-alias-bg-layer-2,#fdf3e3);border:1px solid var(--dsw-alias-border-l2,#f3e3c3);border-radius:8px;margin:0;padding:8px 10px;font-size:12px;line-height:18px}.BOT17a_deprecate a{color:var(--dsw-alias-state-warn-primary,#b45309);text-decoration:underline}.BOT17a_deprecate .BOT17a_src{margin-left:8px}.BOT17a_depLine{flex-wrap:wrap;align-items:center;gap:8px;display:flex}.BOT17a_switch{border:1px solid var(--dsw-alias-border-l2,#d9dde3);background:var(--dsw-alias-bg-layer-2,#e5e7eb);cursor:pointer;border-radius:99px;flex-shrink:0;width:38px;height:22px;padding:0;transition:background .15s,border-color .15s;position:relative}.BOT17a_switchOn{background:var(--dsw-alias-state-success-primary,#16a34a);border-color:var(--dsw-alias-state-success-primary,#16a34a)}.BOT17a_switchMixed{background:var(--dsw-alias-state-warn-primary,#b45309);border-color:var(--dsw-alias-state-warn-primary,#b45309)}.BOT17a_switchKnob{background:#fff;border-radius:99px;width:16px;height:16px;transition:left .15s;position:absolute;top:2px;left:2px;box-shadow:0 1px 2px #00000040}.BOT17a_switchOn .BOT17a_switchKnob,.BOT17a_switchMixed .BOT17a_switchKnob{left:18px}.BOT17a_switch:disabled{opacity:.5;cursor:default}.BOT17a_viewBar{border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:8px;align-items:center;gap:2px;width:fit-content;margin-bottom:12px;padding:2px;display:flex}.BOT17a_viewBtn{font:inherit;color:var(--dsw-alias-label-secondary,#6b7280);cursor:pointer;white-space:nowrap;background:0 0;border:none;border-radius:6px;padding:4px 10px;font-size:12px;line-height:18px}.BOT17a_viewBtn:hover{color:var(--dsw-alias-brand-primary,#4f6ef7)}.BOT17a_viewOn{background:var(--dsw-alias-bg-layer-2,#eef0f4);color:var(--dsw-alias-label-primary,#1f2328);font-weight:600}.BOT17a_groupRow{background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:12px;margin-bottom:10px;padding:12px 14px}.BOT17a_groupHead{align-items:center;gap:10px;min-width:0;display:flex}.BOT17a_groupName{text-overflow:ellipsis;white-space:nowrap;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}.BOT17a_groupActions{flex-shrink:0;align-items:center;gap:6px;display:flex}.BOT17a_groupMembers{flex-direction:column;gap:6px;margin-top:10px;display:flex}.BOT17a_groupMember{background:var(--dsw-alias-bg-layer-2,#f7f8fa);border-radius:8px;align-items:center;gap:8px;padding:6px 8px;font-size:12px;line-height:18px;display:flex}.BOT17a_groupMember .BOT17a_nm{flex:1;min-width:0;font-size:12px}.BOT17a_groupAddPanel{border-top:1px dashed var(--dsw-alias-border-l2,#e5e7eb);flex-direction:column;gap:6px;margin-top:10px;padding-top:10px;display:flex}.BOT17a_groupCreate{align-items:center;gap:8px;margin-bottom:10px;display:flex}.BOT17a_inlineInput{flex:1;min-width:120px}.BOT17a_assignRow{flex-wrap:wrap;align-items:center;gap:8px;display:flex}.BOT17a_assignSelect{border:1px solid var(--dsw-alias-border-l2,#e5e7eb);background:var(--dsw-alias-bg-layer-1,#fff);color:var(--dsw-alias-label-primary,#1f2328);font:inherit;border-radius:6px;padding:3px 6px;font-size:12px;line-height:18px}.BOT17a_groupHint{color:var(--dsw-alias-label-tertiary,#8b93a1);font-size:11px}.BOT17a_sectAction{color:var(--dsw-alias-label-secondary,#6b7280);align-items:center;gap:8px;margin:14px 2px 8px;font-size:12px;font-weight:600;display:flex}.BOT17a_backupGrid{grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;display:grid}.BOT17a_backupCard{background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:12px;flex-direction:column;gap:10px;padding:16px;display:flex}.BOT17a_backupCard h3{margin:0;font-size:14px}.BOT17a_backupCard p{color:var(--dsw-alias-label-secondary,#6b7280);margin:0;font-size:12px;line-height:18px}.BOT17a_backupActions{flex-wrap:wrap;gap:8px;display:flex;position:relative}.BOT17a_hiddenFile{opacity:0;pointer-events:none;width:1px;height:1px;position:absolute}.BOT17a_backupInput{box-sizing:border-box;width:100%}.BOT17a_backupCheck{cursor:pointer;align-items:center;gap:6px;font-size:12px;display:flex}.BOT17a_backupWarn{margin:0;font-size:12px;line-height:18px;color:var(--dsw-alias-state-warn-primary,#b45309)!important}.BOT17a_backupMessage{color:var(--dsw-alias-label-secondary,#6b7280);grid-column:1/-1;font-size:12px}.BOT17a_backupCheckList{flex-direction:column;gap:6px;max-height:260px;margin:10px 0 4px;padding-right:4px;display:flex;overflow-y:auto}.BOT17a_backupCheckList .BOT17a_backupCheck{justify-content:space-between;gap:8px}.BOT17a_diagPage{flex-direction:column;gap:12px;height:100%;min-height:0;display:flex;overflow-y:auto}.BOT17a_diagSummary{background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:12px;flex-wrap:wrap;align-items:center;gap:12px;padding:10px 14px;font-size:12px;display:flex}.BOT17a_diagSummaryItem{color:var(--dsw-alias-label-secondary,#6b7280);white-space:nowrap;align-items:center;gap:6px;display:inline-flex}.BOT17a_diagSummaryMeta{color:var(--dsw-alias-label-tertiary,#9ca3af);text-overflow:ellipsis;white-space:nowrap;max-width:320px;font-family:ui-monospace,Menlo,monospace;font-size:11px;overflow:hidden}.BOT17a_diagSection{background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:12px;flex-direction:column;gap:8px;padding:12px 14px;display:flex}.BOT17a_diagSection h3{color:var(--dsw-alias-label-primary,#1f2328);margin:0;font-size:13px;font-weight:600}.BOT17a_diagCount{color:var(--dsw-alias-label-tertiary,#9ca3af);font-size:11px;font-weight:400}.BOT17a_diagEmpty{color:var(--dsw-alias-label-secondary,#9ca3af);padding:8px 0;font-size:12px}.BOT17a_diagBundle{border-top:1px solid var(--dsw-alias-border-l2,#f0f1f3);flex-direction:column;gap:6px;padding-top:8px;display:flex}.BOT17a_diagBundle:first-of-type{border-top:none;padding-top:0}.BOT17a_diagRow{flex-wrap:wrap;align-items:center;gap:8px;min-width:0;font-size:12px;line-height:18px;display:flex}.BOT17a_diagMeta{align-items:baseline;gap:8px;min-width:0;font-size:12px;display:flex}.BOT17a_diagKey{color:var(--dsw-alias-label-tertiary,#9ca3af);flex-shrink:0;min-width:64px;font-size:11px}.BOT17a_diagVal{color:var(--dsw-alias-label-primary,#1f2328);overflow-wrap:anywhere;min-width:0;font-family:ui-monospace,Menlo,monospace;font-size:12px;font-weight:500}.BOT17a_diagIndex{background:var(--dsw-alias-bg-layer-2,#f3f4f6);min-width:18px;height:18px;color:var(--dsw-alias-label-secondary,#6b7280);border-radius:9px;flex-shrink:0;justify-content:center;align-items:center;font-size:11px;font-weight:600;display:inline-flex}.BOT17a_diagArrow{color:var(--dsw-alias-label-tertiary,#9ca3af);flex-shrink:0;font-size:12px}.BOT17a_diagBadgeOfficial{background:var(--dsw-alias-brand-primary,#4f6ef7);color:#fff;border-radius:9px;flex-shrink:0;align-items:center;height:18px;padding:0 8px;font-size:11px;font-weight:600;display:inline-flex}.BOT17a_diagBadgeCommunity{background:var(--dsw-alias-bg-layer-2,#f3f4f6);height:18px;color:var(--dsw-alias-label-secondary,#6b7280);border-radius:9px;flex-shrink:0;align-items:center;padding:0 8px;font-size:11px;display:inline-flex}.BOT17a_diagBadgeShadow{background:var(--dsw-alias-state-error-primary,#dc2626);color:#fff;border-radius:9px;flex-shrink:0;align-items:center;height:18px;padding:0 8px;font-size:11px;font-weight:600;display:inline-flex}.BOT17a_diagList{flex-direction:column;gap:6px;display:flex}.BOT17a_sectionOverview{color:var(--dsw-alias-label-tertiary,#8b93a1);text-overflow:ellipsis;white-space:nowrap;max-width:100%;padding:2px 0 6px;font-size:12px;line-height:18px;overflow:hidden}.BOT17a_diagAlert{color:var(--dsw-alias-state-warn-primary,#b45309)}.BOT17a_ovRow{background:var(--dsw-alias-bg-layer-2,#f7f8fa);border-radius:8px;flex-wrap:wrap;align-items:center;gap:8px;min-width:0;padding:6px 10px;font-size:12px;line-height:18px;display:flex}.BOT17a_ovArrow{color:var(--dsw-alias-label-tertiary,#9ca3af);flex-shrink:0;font-size:12px}.BOT17a_ovByTag{background:var(--dsw-alias-brand-primary,#4f6ef7);color:#fff;text-overflow:ellipsis;white-space:nowrap;border-radius:9px;flex-shrink:0;align-items:center;max-width:260px;height:18px;padding:0 8px;font-size:11px;font-weight:600;display:inline-flex;overflow:hidden}.BOT17a_ovFrom{color:var(--dsw-alias-label-secondary,#6b7280);text-overflow:ellipsis;white-space:nowrap;min-width:0;font-size:12px;overflow:hidden}.BOT17a_orphRow{background:var(--dsw-alias-bg-layer-2,#f7f8fa);border-radius:8px;flex-wrap:wrap;align-items:center;gap:8px;min-width:0;padding:6px 10px;font-size:12px;line-height:18px;display:flex}.BOT17a_orphBadge{background:var(--dsw-alias-state-warn-primary,#b45309);color:#fff;white-space:nowrap;border-radius:9px;flex-shrink:0;align-items:center;height:18px;padding:0 8px;font-size:11px;font-weight:600;display:inline-flex}.BOT17a_dragHandle{width:20px;height:20px;color:var(--dsw-alias-label-tertiary,#9ca3af);cursor:grab;-webkit-user-select:none;user-select:none;flex-shrink:0;justify-content:center;align-items:center;font-size:12px;line-height:20px;display:inline-flex}.BOT17a_dragOver{outline:2px dashed var(--dsw-alias-brand-primary,#4f6ef7);outline-offset:2px;background:var(--dsw-alias-bg-layer-2,#f0f2f8);border-radius:8px}.BOT17a_dragging{opacity:.45;background:var(--dsw-alias-bg-layer-2,#f3f4f6)}.BOT17a_collapseHead{font:inherit;color:var(--dsw-alias-label-primary,#1f2328);cursor:pointer;text-align:left;background:0 0;border:none;align-items:center;gap:8px;width:100%;padding:0;font-size:13px;font-weight:600;display:flex}.BOT17a_collapseIcon{color:var(--dsw-alias-label-secondary,#6b7280);flex-shrink:0;display:inline-flex}.BOT17a_collapseTitle{flex:1;min-width:0}.BOT17a_collapseBody{border-top:1px solid var(--dsw-alias-border-l2,#f0f1f3);overflow-wrap:anywhere;flex-direction:column;gap:10px;min-width:0;margin-top:8px;padding-top:10px;display:flex}.BOT17a_panelNote{color:var(--dsw-alias-label-secondary,#6b7280);margin:0;font-size:12px;line-height:18px}.BOT17a_fixFallback{flex-direction:column;gap:6px;margin:8px 0;display:flex}.BOT17a_fixFallbackText{box-sizing:border-box;width:100%;color:var(--dsw-alias-label-primary,#1f2328);background:var(--dsw-alias-bg-layer-2,#f3f4f6);border:1px solid var(--dsw-alias-border-l2,#e5e7eb);resize:vertical;white-space:pre-wrap;word-break:break-all;border-radius:6px;padding:8px 10px;font-family:ui-monospace,Menlo,monospace;font-size:11px;line-height:16px}.BOT17a_setCard{border:1px solid var(--dsw-alias-border-l2,#e5e7eb);background:var(--dsw-alias-bg-layer-3,#fff);border-radius:12px;list-style:none;transition:border-color .16s,background .16s}.BOT17a_setCard:hover{border-color:var(--dsw-alias-label-dimmed,#c8ccd4)}.BOT17a_setCardOpen{background:var(--dsw-alias-bg-layer-2,#f7f8fa);border-color:var(--dsw-alias-label-dimmed,#c8ccd4)}.BOT17a_setHeader{-webkit-appearance:none;appearance:none;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:12px;align-items:center;gap:12px;padding:14px 16px;display:flex}.BOT17a_setHeader:focus-visible{outline:2px solid var(--dsw-alias-brand-primary,#4f6ef7);outline-offset:-2px}.BOT17a_setHeadText{flex-direction:column;flex:1;gap:4px;min-width:0;display:flex}.BOT17a_setName{color:var(--dsw-alias-label-primary,#1f2328);font-size:15px;font-weight:600;line-height:1.4}.BOT17a_setDesc{color:var(--dsw-alias-label-tertiary,#8b93a1);font-size:13px;line-height:1.5}.BOT17a_setChevron{color:var(--dsw-alias-label-tertiary,#8b93a1);flex:none;transition:transform .16s;display:inline-flex}.BOT17a_setChevronOpen{transform:rotate(180deg)}.BOT17a_setBody{border-top:1px solid var(--dsw-alias-border-l2,#e5e7eb);margin:0 16px;padding-bottom:8px}.BOT17a_setRow{align-items:center;gap:12px;padding:12px 0;display:flex}.BOT17a_setRow+.BOT17a_setRow{border-top:1px solid var(--dsw-alias-border-l2,#e5e7eb)}.BOT17a_setLabelBox{flex-direction:column;flex:1;gap:3px;min-width:0;display:flex}.BOT17a_setLabel{font-size:13px;line-height:20px}.BOT17a_setHint{color:var(--dsw-alias-label-tertiary,#8b93a1);font-size:12px;line-height:18px}.BOT17a_setConfirm{border-top:1px solid var(--dsw-alias-border-l2,#e5e7eb);flex-direction:column;gap:8px;padding:12px 0 4px;display:flex}.BOT17a_setCheck{cursor:pointer;align-items:center;gap:8px;font-size:12px;line-height:18px;display:flex}.BOT17a_setActions{border-top:1px solid var(--dsw-alias-border-l2,#e5e7eb);justify-content:flex-end;align-items:center;gap:8px;padding:12px 0 4px;display:flex}.BOT17a_setDanger{color:var(--dsw-alias-state-error-primary,#dc2626)}.BOT17a_setSeg{border:1px solid var(--dsw-alias-border-l2,#e5e7eb);border-radius:8px;flex-shrink:0;gap:2px;padding:2px;display:inline-flex}.BOT17a_setSegBtn{font:inherit;color:var(--dsw-alias-label-secondary,#6b7280);cursor:pointer;background:0 0;border:none;border-radius:6px;padding:3px 10px;font-size:12px;line-height:18px}.BOT17a_setSegBtn:disabled{cursor:default;opacity:.5}.BOT17a_setSegOn{background:var(--dsw-alias-bg-layer-2,#eef0f4);color:var(--dsw-alias-label-primary,#1f2328);font-weight:600}.BOT17a_setBetaTag{background:var(--dsw-alias-bg-module-platform,#eef0f4);color:var(--dsw-alias-label-secondary,#6b7280);border-radius:9px;margin-left:6px;padding:0 6px;font-size:11px;font-weight:600;line-height:17px}";
		...
		var Market_module_css_default = {
			"act": "BOT17a_act",
			"actBroken": "BOT17a_actBroken",
			"actLive": "BOT17a_actLive",
			"actWarn": "BOT17a_actWarn",
			"actWhy": "BOT17a_actWhy",
			"assignRow": "BOT17a_assignRow",
			"assignSelect": "BOT17a_assignSelect",
			"av": "BOT17a_av",
			"backupActions": "BOT17a_backupActions",
			"backupCard": "BOT17a_backupCard",
			"backupCheck": "BOT17a_backupCheck",
			"backupCheckList": "BOT17a_backupCheckList",
			"backupGrid": "BOT17a_backupGrid",
			"backupInput": "BOT17a_backupInput",
			"backupMessage": "BOT17a_backupMessage",
			"backupWarn": "BOT17a_backupWarn",
			"banner": "BOT17a_banner",
			"bannerHint": "BOT17a_bannerHint",
			"bannerIcon": "BOT17a_bannerIcon",
			"bar": "BOT17a_bar",
			"barFill": "BOT17a_barFill",
			"barWave": "BOT17a_barWave",
			"body": "BOT17a_body",
			"byline": "BOT17a_byline",
			"card": "BOT17a_card",
			"cardAction": "BOT17a_cardAction",
			"cardBlocked": "BOT17a_cardBlocked",
			"cardBlockedMark": "BOT17a_cardBlockedMark",
			"cardShot": "BOT17a_cardShot",
			"cardShots": "BOT17a_cardShots",
			"cats": "BOT17a_cats",
			"catsCollapsed": "BOT17a_catsCollapsed",
			"catsRow": "BOT17a_catsRow",
			"catsToggle": "BOT17a_catsToggle",
			"catsWrap": "BOT17a_catsWrap",
			"choice": "BOT17a_choice",
			"choiceMain": "BOT17a_choiceMain",
			"choiceNote": "BOT17a_choiceNote",
			"choiceOn": "BOT17a_choiceOn",
			"choiceRadio": "BOT17a_choiceRadio",
			"choices": "BOT17a_choices",
			"choiceSafe": "BOT17a_choiceSafe",
			"choiceTitle": "BOT17a_choiceTitle",
			"cmd": "BOT17a_cmd",
			"collapseBody": "BOT17a_collapseBody",
			"collapseHead": "BOT17a_collapseHead",
			"collapseIcon": "BOT17a_collapseIcon",
			"collapseTitle": "BOT17a_collapseTitle",
			"conflictBody": "BOT17a_conflictBody",
			"conflictDetailsToggle": "BOT17a_conflictDetailsToggle",
			"conflictHead": "BOT17a_conflictHead",
			"conflictIcon": "BOT17a_conflictIcon",
			"conflictTitle": "BOT17a_conflictTitle",
			"conflictWhy": "BOT17a_conflictWhy",
			"conflictWhyText": "BOT17a_conflictWhyText",
			"dangerArmed": "BOT17a_dangerArmed",
			"dangerBtn": "BOT17a_dangerBtn",
			"depBadge": "BOT17a_depBadge",
			"depLine": "BOT17a_depLine",
			"deprecate": "BOT17a_deprecate",
			"desc": "BOT17a_desc",
			"descClamp": "BOT17a_descClamp",
			"descTight": "BOT17a_descTight",
			"descToggle": "BOT17a_descToggle",
			"diagAlert": "BOT17a_diagAlert",
			"diagArrow": "BOT17a_diagArrow",
			"diagBadgeCommunity": "BOT17a_diagBadgeCommunity",
			"diagBadgeOfficial": "BOT17a_diagBadgeOfficial",
			"diagBadgeShadow": "BOT17a_diagBadgeShadow",
			"diagBundle": "BOT17a_diagBundle",
			"diagCount": "BOT17a_diagCount",
			"diagEmpty": "BOT17a_diagEmpty",
			"diagIndex": "BOT17a_diagIndex",
			"diagKey": "BOT17a_diagKey",
			"diagList": "BOT17a_diagList",
			"diagMeta": "BOT17a_diagMeta",
			"diagPage": "BOT17a_diagPage",
			"diagRow": "BOT17a_diagRow",
			"diagSection": "BOT17a_diagSection",
			"diagSummary": "BOT17a_diagSummary",
			"diagSummaryItem": "BOT17a_diagSummaryItem",
			"diagSummaryMeta": "BOT17a_diagSummaryMeta",
			"diagVal": "BOT17a_diagVal",
			"dot": "BOT17a_dot",
			"dragging": "BOT17a_dragging",
			"dragHandle": "BOT17a_dragHandle",
			"dragOver": "BOT17a_dragOver",
			"dshmPlug": "BOT17a_dshmPlug",
			"dshmPlugFade": "BOT17a_dshmPlugFade",
			"dshmSlide": "BOT17a_dshmSlide",
			"empty": "BOT17a_empty",
			"err": "BOT17a_err",
			"fixFallback": "BOT17a_fixFallback",
			"fixFallbackText": "BOT17a_fixFallbackText",
			"foot": "BOT17a_foot",
			"grid": "BOT17a_grid",
			"groupActions": "BOT17a_groupActions",
			"groupAddPanel": "BOT17a_groupAddPanel",
			"groupCreate": "BOT17a_groupCreate",
			"groupHead": "BOT17a_groupHead",
			"groupHint": "BOT17a_groupHint",
			"groupMember": "BOT17a_groupMember",
			"groupMembers": "BOT17a_groupMembers",
			"groupName": "BOT17a_groupName",
			"groupRow": "BOT17a_groupRow",
			"grow": "BOT17a_grow",
			"head": "BOT17a_head",
			"hiddenFile": "BOT17a_hiddenFile",
			"inlineInput": "BOT17a_inlineInput",
			"installBtn": "BOT17a_installBtn",
			"irow": "BOT17a_irow",
			"irowActions": "BOT17a_irowActions",
			"irowMissing": "BOT17a_irowMissing",
			"irowName": "BOT17a_irowName",
			"irowNameText": "BOT17a_irowNameText",
			"irowTrailing": "BOT17a_irowTrailing",
			"lightbox": "BOT17a_lightbox",
			"lightboxClose": "BOT17a_lightboxClose",
			"lightboxDot": "BOT17a_lightboxDot",
			"lightboxDotOn": "BOT17a_lightboxDotOn",
			"lightboxDots": "BOT17a_lightboxDots",
			"lightboxImg": "BOT17a_lightboxImg",
			"lightboxNav": "BOT17a_lightboxNav",
			"lightboxNext": "BOT17a_lightboxNext",
			"lightboxPrev": "BOT17a_lightboxPrev",
			"loading": "BOT17a_loading",
			"logoMark": "BOT17a_logoMark",
			"logoPlug": "BOT17a_logoPlug",
			"masonry": "BOT17a_masonry",
			"masonryCol": "BOT17a_masonryCol",
			"meta": "BOT17a_meta",
			"metaInline": "BOT17a_metaInline",
			"metaTag": "BOT17a_metaTag",
			"metaTagOk": "BOT17a_metaTagOk",
			"modalNote": "BOT17a_modalNote",
			"nameLink": "BOT17a_nameLink",
			"nm": "BOT17a_nm",
			"nmLink": "BOT17a_nmLink",
			"okState": "BOT17a_okState",
			"on": "BOT17a_on",
			"opActions": "BOT17a_opActions",
			"opAggregate": "BOT17a_opAggregate",
			"opAggregateHint": "BOT17a_opAggregateHint",
			"opAggregateTop": "BOT17a_opAggregateTop",
			"opCloseBtn": "BOT17a_opCloseBtn",
			"opDecision": "BOT17a_opDecision",
			"opDecisionFoot": "BOT17a_opDecisionFoot",
			"opDot": "BOT17a_opDot",
			"opEmpty": "BOT17a_opEmpty",
			"opEmptyHint": "BOT17a_opEmptyHint",
			"opEntry": "BOT17a_opEntry",
			"opEntryAlert": "BOT17a_opEntryAlert",
			"opEntryQuiet": "BOT17a_opEntryQuiet",
			"opHead": "BOT17a_opHead",
			"opIcon": "BOT17a_opIcon",
			"opMain": "BOT17a_opMain",
			"opName": "BOT17a_opName",
			"opPanel": "BOT17a_opPanel",
			"opPanelTitle": "BOT17a_opPanelTitle",
			"opQueuedIcon": "BOT17a_opQueuedIcon",
			"opRow": "BOT17a_opRow",
			"opRowAlert": "BOT17a_opRowAlert",
			"opStatus": "BOT17a_opStatus",
			"opStatusBad": "BOT17a_opStatusBad",
			"opTop": "BOT17a_opTop",
			"opVerb": "BOT17a_opVerb",
			"opWrap": "BOT17a_opWrap",
			"orphBadge": "BOT17a_orphBadge",
			"orphRow": "BOT17a_orphRow",
			"ovArrow": "BOT17a_ovArrow",
			"ovByTag": "BOT17a_ovByTag",
			"ovFrom": "BOT17a_ovFrom",
			"ovRow": "BOT17a_ovRow",
			"owner": "BOT17a_owner",
			"pageEllipsis": "BOT17a_pageEllipsis",
			"pageInfo": "BOT17a_pageInfo",
			"pager": "BOT17a_pager",
			"pagerPages": "BOT17a_pagerPages",
			"pairGrid": "BOT17a_pairGrid",
			"panelNote": "BOT17a_panelNote",
			"pct": "BOT17a_pct",
			"progress": "BOT17a_progress",
			"reassure": "BOT17a_reassure",
			"reassureOk": "BOT17a_reassureOk",
			"repoLink": "BOT17a_repoLink",
			"retryBtn": "BOT17a_retryBtn",
			"root": "BOT17a_root",
			"roster": "BOT17a_roster",
			"rosterAuthor": "BOT17a_rosterAuthor",
			"rosterMain": "BOT17a_rosterMain",
			"rosterName": "BOT17a_rosterName",
			"rosterRow": "BOT17a_rosterRow",
			"rosterRowOut": "BOT17a_rosterRowOut",
			"rosterSplit": "BOT17a_rosterSplit",
			"rosterTag": "BOT17a_rosterTag",
			"rosterTagDrop": "BOT17a_rosterTagDrop",
			"rosterTagKeep": "BOT17a_rosterTagKeep",
			"row1": "BOT17a_row1",
			"sectAction": "BOT17a_sectAction",
			"sectionOverview": "BOT17a_sectionOverview",
			"setActions": "BOT17a_setActions",
			"setBetaTag": "BOT17a_setBetaTag",
			"setBody": "BOT17a_setBody",
			"setCard": "BOT17a_setCard",
			"setCardOpen": "BOT17a_setCardOpen",
			"setCheck": "BOT17a_setCheck",
			"setChevron": "BOT17a_setChevron",
			"setChevronOpen": "BOT17a_setChevronOpen",
			"setConfirm": "BOT17a_setConfirm",
			"setDanger": "BOT17a_setDanger",
			"setDesc": "BOT17a_setDesc",
			"setHeader": "BOT17a_setHeader",
			"setHeadText": "BOT17a_setHeadText",
			"setHint": "BOT17a_setHint",
			"setLabel": "BOT17a_setLabel",
			"setLabelBox": "BOT17a_setLabelBox",
			"setName": "BOT17a_setName",
			"setRow": "BOT17a_setRow",
			"setSeg": "BOT17a_setSeg",
			"setSegBtn": "BOT17a_setSegBtn",
			"setSegOn": "BOT17a_setSegOn",
			"shot": "BOT17a_shot",
			"shots": "BOT17a_shots",
			"sp": "BOT17a_sp",
			"spec": "BOT17a_spec",
			"specTag": "BOT17a_specTag",
			"specTagFile": "BOT17a_specTagFile",
			"specTagGit": "BOT17a_specTagGit",
			"spin": "BOT17a_spin",
			"src": "BOT17a_src",
			"staleAction": "BOT17a_staleAction",
			"star": "BOT17a_star",
			"stateDot": "BOT17a_stateDot",
			"stateTag": "BOT17a_stateTag",
			"stickyHead": "BOT17a_stickyHead",
			"sub": "BOT17a_sub",
			"submitLink": "BOT17a_submitLink",
			"subTabs": "BOT17a_subTabs",
			"swatches": "BOT17a_swatches",
			"switch": "BOT17a_switch",
			"switchKnob": "BOT17a_switchKnob",
			"switchMixed": "BOT17a_switchMixed",
			"switchOn": "BOT17a_switchOn",
			"tab": "BOT17a_tab",
			"tabs": "BOT17a_tabs",
			"tabSearch": "BOT17a_tabSearch",
			"tabSearchRow": "BOT17a_tabSearchRow",
			"tag": "BOT17a_tag",
			"themesGrid": "BOT17a_themesGrid",
			"title": "BOT17a_title",
			"titleRow": "BOT17a_titleRow",
			"top": "BOT17a_top",
			"topBtn": "BOT17a_topBtn",
			"version": "BOT17a_version",
			"viewBar": "BOT17a_viewBar",
			"viewBtn": "BOT17a_viewBtn",
			"viewOn": "BOT17a_viewOn",
			"warnLine": "BOT17a_warnLine"
		};
		//#endregion
