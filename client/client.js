window.__ModuleLoader__.load({ id: "dshmarket", factory: (require) => {
		...
		const zh = {
			...
			aiFixIfSelf: "【是 → 只给计划，严禁改动】你正在修复的正是自己运行其中的 harness：严禁任何改动运行中组合、或触碰 harness / 核心包本身的操作——包括但不限于改写 cordis.patch.yml、增删或重排 dsh.profile.bundles、停用/启用 bundle、安装/更新/移除插件、重装（pnpm install / reinstall）依赖、升级或重启 DeepSeek Harness 及其任何 @deepseek-ai/* 核心包。只能输出一份【修复方案】：给出逐条准确、可放到外部终端手动执行的命令；每一步说明理由、预期影响与风险，等我逐条确认后再继续。",
