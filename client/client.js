window.__ModuleLoader__.load({ id: "dshmarket", factory: (require) => {
		...
		const zh = {
			...
			orderNoRules: "没有可用的排序规则（插件未声明 before/after，也无插件间依赖）",
		...
		const en = {
			...
			orderNoRules: "No ordering rules available (plugins declare no before/after and have no inter-plugin dependencies)",
			...
			*/
			const startAgentFix = () => {
				...
				try {
					...
					(dialog !== null ? [...dialog.querySelectorAll("button")].find((button) => /关闭|Close|✕|×/.test(`${button.getAttribute("aria-label") ?? ""} ${button.textContent ?? ""}`)) : null)?.click();
				} catch {}
