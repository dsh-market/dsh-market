...
- 一键重启会启动脱离终端的替代进程。**检测到进程管理器（systemd、pm2）时按钮会自动隐藏**——否则市场重启会连带杀掉进程组里的接管进程，服务起不来。待重启提示仍会显示，并会说明检测到的是哪个管理器。launchd 没有可识别的标记，这类部署仍需下面的显式配置。两种做法：在**设置 → 插件 → 插件配置**里关掉「允许重启」，或者写进 profile 补丁——注意必须嵌在 `config:` 下面，因为 loader 只把这个子对象传给插件，写在顶层会静默失效（#227，感谢 @Fantasymax）：
