...
- One-click restart launches a detached replacement. **Under a detected supervisor (systemd, pm2) the button is hidden automatically** — the market would otherwise kill the takeover process along with the unit's process group and the service would not come back. The pending-change notice stays visible and explains which supervisor was detected. launchd sets no distinctive marker, so a launchd deployment still needs the explicit setting below. Either flip **Allow restart** off in **Settings → Plugins → Plugin configuration**, or write it into the profile patch — where it has to sit under `config:`, because the loader passes only that sub-object to a plugin and a top-level `allowRestart:` is silently ignored (#227 by @Fantasymax):
